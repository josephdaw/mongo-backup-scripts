"""Module to copy Production to Local MongoDB.
This will copy the "production" database down from the MongoDB Cloud. 
It will then restore that into the local MongoDB instance as "utm-production"."""
...
backup_mongodb_uri = os.getenv("UTM_MAIN_PRODUCTION")
...
def backup_production_to_local():
    """Back up the production database from MongoDB Cloud to a local directory."""
    create_backup(backup_mongodb_uri, "production", backup_directory)


def restore_production_to_local():
    """Restore the production database from a local backup to the local MongoDB instance."""
    restore_backup(restore_mongodb_uri, "utm-production", backup_directory, "production")
